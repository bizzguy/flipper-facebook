...
parser.add_argument('-l', '--min-level', dest='min_level', type=str, choices=LOG_LEVELS+LOG_LEVELS.lower(), default='E', help='Minimum level to be displayed')
...
#print args
#print('ADB Command %s') % (base_adb_command)
...
#print('catchall_package %s') % (catchall_package)
#print('named_processes %s') % (named_processes)
...
# Regular expressions to match against log message
...
#print('adb_command %s') % (adb_command)
#exit()
...
def parse_start_proc(line):
  ...
  if start is not None:
    ...
    #print('line_pid: %s') % (line_pid)
    #print('line_package: %s') % (line_package)
...
while adb.poll() is None:
  ...
  if start:
    ...
    if line_package == "com.footlocker.approved":
      print('line: %s') % (line)
