...
import type {Counter} from './Adobe.js';
import type {NameValuePair} from './Adobe.js';
import type {DeviceLogEntry} from '../../devices/BaseDevice.js';
import type {Props as PluginProps} from '../../plugin';
import {formatDate, getMessageValue, trimStartEndChars, pad, getEntryType, getTestDataRow, filterLogMessage} from './utils.js';
...
export function processEntry(entry: DeviceLogEntry, key: string): {row: TableBodyRow, entry: DeviceLogEntry} {
  ...
  // calculate hit value
  let messageValue = getMessageValue(entry.message)
  ...
  return {
    row: {
      columns: {
        ...
        hitColumn: {
          value: (
            <HiddenScrollText code={true}>{messageValue}</HiddenScrollText>
          ),
        },
        tag: {
